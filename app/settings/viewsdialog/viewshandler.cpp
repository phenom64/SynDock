...
void ViewsHandler::initViewTemplatesSubMenu()
{
    ...
    if (templates.rowCount() > 0) {
        ...
        connect(openTemplatesDirectory, &QAction::triggered, this, [&]() {
            KIO::highlightInFileManager({QString(NSE::dataPath() + "/syndock/templates/Dock.view.latte")});
        });
    }
}
...
void ViewsHandler::exportViewForBackup()
{
    ...
    QStringList filters;
    QString filter1(i18nc("export view", "SynDock Dock/Panel file v0.2") + "(*.view.latte)");
...
void ViewsHandler::importView()
{
    ...
    QStringList filters;
    filters << QString(i18nc("import dock panel", "SynDock Dock or Panel file v0.2") + "(*.view.latte)");
