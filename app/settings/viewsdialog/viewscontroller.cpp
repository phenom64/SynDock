...
void Views::save()
{
    ...
    m_debugSaveCall++;
    qDebug() << "org.syndromatic.syndock ViewsDialog::save() call: " << m_debugSaveCall << "-------- ";
    ...
    for (int i=0; i<alteredViews.rowCount(); ++i) {
        if (alteredViews[i].state() == Data::View::IsCreated && !alteredViews[i].isMoveOrigin) {
            qDebug() << "org.syndromatic.syndock ViewsDialog::save() updating altered view :: " << alteredViews[i];
    ...
    for (int i=0; i<removedViews.rowCount(); ++i) {
        qDebug() << "org.syndromatic.syndock ViewsDialog::save() real removing view :: " << removedViews[i];
    ...
    for(const auto vid: cuttedpastedviews.keys()){
        ...
        qDebug() << "org.syndromatic.syndock ViewsDialog::save() removing cut-pasted view :: " << cuttedpastedviews[vid];
    ...
    for (const auto vid: cuttedpastedactiveviews.keys()) {
        ...
        qDebug() << "org.syndromatic.syndock ViewsDialog::save() move to another layout cutted-pasted active view :: " << pastedactiveview;
