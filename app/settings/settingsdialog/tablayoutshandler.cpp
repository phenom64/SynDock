...
void TabLayouts::initUi()
{
    ...
    connect(m_corona->activitiesConsumer(), &KActivities::Consumer::activitiesChanged, this, [this](const QStringList &) {
...
void TabLayouts::initLayoutTemplatesSubMenu()
{
    ...
    if (templates.rowCount() > 0) {
        ...
        connect(openTemplatesDirectory, &QAction::triggered, this, [&]() {
            KIO::highlightInFileManager({QString(NSE::dataPath() + "/syndock/templates/Dock.layout.latte")});
        });
    }
}
...
void TabLayouts::downloadLayout()
{
    ...
    KNSWidgets::Dialog dialog(QStringLiteral("syndock-layouts.knsrc"), m_parentDialog);
