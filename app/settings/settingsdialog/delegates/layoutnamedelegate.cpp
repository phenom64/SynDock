...
void LayoutName::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    //! Changes Indicator
    QRect remainedrect = NSE::remainedFromChangesIndicator(myOptions);
    if (isChanged) {
    ...
    if (isConsideredActive) {
        remainedrect = NSE::remainedFromIcon(myOptions, Qt::AlignRight, -1, 1);
        NSE::drawIcon(painter, myOptions, "favorite", Qt::AlignRight, -1, 1);
        myOptions.rect = remainedrect;
    }

    if (isLocked) {
        remainedrect = NSE::remainedFromIcon(myOptions, Qt::AlignRight, -1, 1);
        NSE::drawIcon(painter, myOptions, "object-locked", Qt::AlignRight, -1, 1);
        myOptions.rect = remainedrect;
    }
