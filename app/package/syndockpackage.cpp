...
void Package::initPackage(KPackage::Package *package)
{
    ...
    package->setPath("org.syndromatic.syndock.shell");
    package->addFileDefinition("defaults", QStringLiteral("defaults"));
    package->addFileDefinition("syndockui", QStringLiteral("views/Panel.qml"));
    package->addFileDefinition("widgetexplorerui", QStringLiteral("views/WidgetExplorer.qml"));
    //Configuration
    package->addFileDefinition("syndockconfigurationui", QStringLiteral("configuration/LatteDockConfiguration.qml"));
    package->addFileDefinition("syndocksecondaryconfigurationui", QStringLiteral("configuration/LatteDockSecondaryConfiguration.qml"));
...
void Package::pathChanged(KPackage::Package *package)
{
    ...
    if (!pluginName.isEmpty() && pluginName != "org.syndromatic.syndock.shell") {
        auto fallback = KPackage::PackageLoader::self()->loadPackage("Plasma/Shell", "org.syndromatic.syndock.shell");
        package->setFallbackPackage(fallback);
    } else if (pluginName.isEmpty() || pluginName == QLatin1String("org.syndromatic.syndock.shell")) {
